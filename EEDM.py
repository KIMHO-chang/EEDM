...
def run_pre_model(train_data, test_data, y_train, y_test, params):
    X_train = []
    X_test = []
    for mode in modes:
    ...
    if not tf.io.gfile.exists(MODEL_FILE):
        ...
        model.compile(loss=objective, optimizer=optimizers.RMSprop(learning_rate=params['lr_pre']), metrics=metric, run_eagerly=True)
        model.fit(X_train_last, y_train_merge, batch_size=params['batch_size'], verbose=2, epochs=int(params['n_epochs'] * 0.5), validation_split=0.2, validation_freq=1, )
    ...
    return y_pred, model, X_train_last, y_train_merge, X_test
...
def run_model(model, X_train, y_train, X_test):
    ...
    def loss_fn_EEDM(y_true, y_pred, x):
        ...
        g_al_output = g_al(x[0])
        g_sp_output = g_sp(x[1])
        g_ac_output = g_ac(x[2])
        f_input = np.concatenate((g_al_output, g_sp_output, g_ac_output), axis=1)
        explainer = shap.DeepExplainer(f, f_input)
        shap_values = np.abs(explainer.shap_values(f_input)[0])
        r_loss = ranking_loss(shap_values)
        l2 = r_loss
        l2 = tf.cast(l2, dtype=tf.float32)
        return l1 + l2

    metric = [acc] if params['is_clf'] else [rmse]
    model.compile(loss=loss_fn_EEDM, optimizer=optimizers.RMSprop(learning_rate=params['lr']), metrics=metric, run_eagerly=True)
    model.fit(X_train, y_train, batch_size=params['batch_size'], verbose=2, epochs=int(params['n_epochs'] * 0.5), validation_split=0.2, validation_freq=1, )
    model.summary()
    y_score = model.predict(X_test, batch_size=params['batch_size'], verbose=0)
    y_pred = (np.ravel(y_score) > 0.5).astype('int32') if params['is_clf'] else np.ravel(y_score)
    return y_pred
...
params = {'seq_min_len': 10,
          ...
          'accel_feature_num': 3,
          'flag': 1}
subject_ids = [7, 13, 15, 16, 17, 19, 20, 21, 24, 25, 27, 30, 31, 32, 33, 36, 37, 38, 39, 40]  # 受试者编号
MODEL_DIR = './model/'
MODEL_FILE = './model/pre_model_(data_size).h5'


train_data, y_train, test_data, y_test = split_data(subject_ids, params)
y_pred_pre, modele, X_train, y_train, X_test = run_pre_model(train_data, test_data, y_train, y_test, params)
res_pre = evaluate(y_test, y_pred_pre, params)
y_pred = run_model(modele, X_train, y_train, X_test)
